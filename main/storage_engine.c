...
static void get_event_path(const uint8_t event_id[32], uint16_t file_index,
                           char *path, size_t len)
{
    char id_hex[33];
    nostr_bytes_to_hex(event_id, 16, id_hex);
    snprintf(path, len, EVENTS_DIR "/%02x/%s_%04x.bin",
...
static int save_index_to_nvs(storage_engine_t *engine)
{
    ...
    nvs_set_u16(nvs, "next_idx", engine->next_file_index);
    ...
    for (uint16_t i = 0; i < engine->index_count; i += chunk_size) {
        ...
        nvs_set_blob(nvs, key, &engine->index[i],
...
static int load_index_from_nvs(storage_engine_t *engine)
{
    ...
    nvs_get_u16(nvs, "count", &engine->index_count);
    nvs_get_u16(nvs, "next_idx", &engine->next_file_index);

    const uint16_t chunk_size = 50;
    for (uint16_t i = 0; i < engine->index_count; i += chunk_size) {
        char key[16];
        snprintf(key, sizeof(key), "idx_%u", i / chunk_size);
        size_t len = chunk_size * sizeof(storage_index_entry_t);
        nvs_get_blob(nvs, key, &engine->index[i], &len);
    }

    nvs_close(nvs);
    ESP_LOGI(TAG, "Loaded %u index entries", engine->index_count);
    return STORAGE_OK;
}
...
esp_err_t storage_init(storage_engine_t *engine, uint32_t default_ttl_sec)
{
    ...
    load_index_from_nvs(engine);

    engine->initialized = true;

    size_t total, used;
    esp_littlefs_info(STORAGE_PARTITION_LABEL, &total, &used);
    ESP_LOGI(TAG, "Storage initialized: %u events, %zu/%zu bytes used",
...
storage_error_t storage_save_event(storage_engine_t *engine, const nostr_event *event)
{
    ...
    char json[STORAGE_MAX_EVENT_SIZE];
    size_t json_len;
    nostr_relay_error_t ser_err = nostr_event_serialize(event, json, sizeof(json), &json_len);
    ...
    ESP_LOGD(TAG, "Stored event: kind=%u, expires=%u", event->kind, entry->expires_at);
    return STORAGE_OK;
}
...
static nostr_event *load_event_from_file(const char *path)
{
    ...
    nostr_event_parse(json, size, &event);
    free(json);
                                     ...
                                     uint16_t limit)
{
    ...
    *results = events;
    ESP_LOGD(TAG, "Query returned %u events", *count);
    return STORAGE_OK;
}
...
int storage_compact_index(storage_engine_t *engine)
{
    ...
    if (compacted > 0) {
        ...
        save_index_to_nvs(engine);
        ESP_LOGI(TAG, "Compacted index: removed %d entries, %u remaining",
...
void storage_get_stats(storage_engine_t *engine, storage_stats_t *stats)
{
    ...
    for (uint16_t i = 0; i < engine->index_count; i++) {
        ...
        if (engine->index[i].expires_at < now) continue;
